...
alertsfile="/tmp/mdiag-alerts-`hostname`.txt"
...
[ -e "$alertsfile" ] && mv -f "$alertsfile" "$alertsfile.old"
(
echo "========================="
echo "MongoDB Diagnostic Alerts"
echo "========================="
) > "$alertsfile" 2>&1

# Essential checks
msection_check thp << EOF
if [ -d /sys/kernel/mm/transparent_hugepage ]; then
   grep '\[never\]' /sys/kernel/mm/transparent_hugepage/enabled >/dev/null || echo "THP are enabled!"
   grep '\[never\]' /sys/kernel/mm/transparent_hugepage/defrag >/dev/null || echo "THP-defrag is enabled!"
fi
if [ -d /sys/kernel/mm/redhat_transparent_hugepage/enabled ]; then
   grep '\[never\]' /sys/kernel/mm/redhat_transparent_hugepage/enabled >/dev/null || echo "RH THP are enabled!"
   grep '\[never\]' /sys/kernel/mm/redhat_transparent_hugepage/defrag >/dev/null || echo "RH THP-defrag is enabled!"
fi
EOF

msection_check mount << EOF
mount | grep nodiratime | grep atime >/dev/null || echo "Can't find noatime and nodiratime in mount"
mount | grep swap >/dev/null || echo "Can't find swap space listed in mount"
EOF

msection_check ulimits << EOF
mongo_pids="`pgrep mongo`"
for pid in \$mongo_pids; do
    echo "Checking pid \$pid: \`ps -w -p \$pid -o command=\`"
    grep "Max processes" /proc/\$pid/limits | perl -nle 'm/Max processes[ ]+(\d*)/; if (\$1 < 64000) {print "Soft limit for number of processes \$1 is too low!"}'
    grep "Max processes" /proc/\$pid/limits | perl -nle 'm/Max processes[ ]+\d*[ ]+(\d*)/; if (\$1 < 64000) {print "Hard limit for number of processes \$1 is too low!"}'
    grep "Max open files" /proc/\$pid/limits | perl -nle 'm/Max open files[ ]+(\d*)/; if (\$1 < 64000) {print "Soft limit for number of open files \$1 is too low!"}'
    grep "Max open files" /proc/\$pid/limits | perl -nle 'm/Max open files[ ]+\d*[ ]+(\d*)/; if (\$1 < 64000) {print "Hard limit for number of open files \$1 is too low!"}'    
done
EOF

msection_check syslimits << EOF
    cat /proc/sys/fs/file-max | perl -nle 'm/(\d*)/; if (\$1 < 98000) {print "Kernel max open files limit \$1 is too low!"}'
    cat /proc/sys/kernel/pid_max | perl -nle 'm/(\d*)/; if (\$1 <= 32768) {print "Kernel max pid limit \$1 might be too low! Consider increasing it if you are running multiple mongods"}'
    cat /proc/sys/kernel/threads-max | perl -nle 'm/(\d*)/; if (\$1 <= 64000) {print "Kernel max thread limit \$1 might be too low! Consider increasing it or adding more RAM"}'
EOF

numa=`dmesg | grep -i numa`
msection_check numa << EOF
    if [[ "x$numa" != "x" && "$numa" != "No NUMA configuration found" ]]; then echo "NUMA is ON"; fi
    cat /proc/sys/vm/zone_reclaim_mode | perl -nle 'm/(\d*)/; if (\$1 == 1) {print "zone_reclaim_mode is not off!"}'
EOF

msection_check readahead << EOF
    blockdev --report | perl -nle 'if (m/[^ ]+[ ]+(\d+)[ ]+\d+[ ]+\d+[ ]+\d+[ ]+\d+[ ]+([^ ]+)/) { if (\$1 > 64) {print "Readahead \$1 for \$2 might be too high!"}}'
EOF

msection_check dmesg_fails << EOF
    dmesg | egrep -i "error|fail|warn|blocked"
EOF
...
Alerts have been recorded into $alertsfile
Please attach the contents of $diagfile and $alertsfile to the Jira ticket${1+ at:
