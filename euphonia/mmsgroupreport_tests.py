...
class MmsGroupReportTests:
    # MmsGroupReport := mmsgr for short
    ...
    def testEvenVotingNumberReplicaSets(mmsgr):
        if mmsgr.doc['NumEvenVotingNumberReplicaSets'] and\
                mmsgr.doc['NumEvenVotingNumberReplicaSets'] != 0:
            return {'pass': False, 'ids': [mmsgr.doc['_id']]}
        return {'pass': True}

    # This is a test for ulimit-related warnings on startup
    def testLimitsStartupWarning(mmsgr):
        if mmsgr.doc['NumActiveHostWithRlimitsStartupWarning'] and\
                mmsgr.doc['NumActiveHostWithRlimitsStartupWarning'] != 0:
            return {'pass': False, 'ids': [mmsgr.doc['_id']]}
        return {'pass': True}
    ...
    def testMongo20ReplSetStepDown(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 2.0.1
    ...
    def testMongo22Idempotency(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # 2.2.0
    ...
    # SERVER-9983
    def testMongo24AuthDbLock(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 2.4.4
    ...
    def testMongo24DbhashCache(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 2.4.7
    ...
    # engine: SERVER-9878
    def testMongo24JSRemoteSegfault(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 2.4.4
    ...
    # SERVER-9087
    def testMongo24InitialSync(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # 2.4.0
    ...
    def testMongo24SecondaryIndexes(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # 2.4.0
    ...
    def testMongo26Credentials(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # 2.6.0
    ...
    # these releases?
    def testMongoJSShellConflicts(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 2.2.3
    ...
    def testMongoLargeChunkMigrations(mmsgr):
        if mmsgr.doc['NumActiveShardedClusters'] > 0 and\
                mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 2.2.5
    ...
    def testMongoSecondaryMissingDocs(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 1.8.4
    ...
    def testMongoSMNativeHelper(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 2.0.8
    ...
    def testMongoTextIndexedFields(mmsgr):
        if mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
                # <= 2.4.10
    ...
    def testMongoWritebackListener(mmsgr):
        if mmsgr.doc['NumActiveShardedClusters'] > 0 and\
                mmsgr.doc['MongoVersion']:
            vers = mmsgr.doc['MongoVersion'].split('|')
            for ver in vers:
    ...
    def testNMonitoringAgents(mmsgr):
        if mmsgr.doc['NumActiveAgent'] and\
                mmsgr.doc['NumActiveAgent'] > 5:
            return {'pass': False, 'ids': [mmsgr.doc['_id']]}
        return {'pass': True}

    # This is a test for NUMA-related warnings on startup
    def testNumaStartupWarning(mmsgr):
        if mmsgr.doc['NumActiveHostWithNumaStartupWarning'] and\
                mmsgr.doc['NumActiveHostWithNumaStartupWarning'] != 0:
            return {'pass': False, 'ids': [mmsgr.doc['_id']]}
        return {'pass': True}

    # This is a test for warnings on startup. It is inclusive to the NUMA and
    # Limits tests
    def testStartupWarning(mmsgr):
        if mmsgr.doc['NumActiveHostWithStartupWarning'] and\
                mmsgr.doc['NumActiveHostWithStartupWarning'] != 0:
            return {'pass': False, 'ids': [mmsgr.doc['_id']]}
        return {'pass': True}

    def testNumReplicaSetWithMoreThanOneArbiter(mmsgr):
        if mmsgr.doc['NumReplicaSetWithMoreThanOneArbiter'] and\
                mmsgr.doc['NumReplicaSetWithMoreThanOneArbiter'] != 0:
            return {'pass': False, 'ids': [mmsgr.doc['_id']]}
        return {'pass': True}

    def testNumHostWithVotesMoreThanOne(mmsgr):
        if mmsgr.doc['NumHostWithMoreThanOneVote'] and\
                mmsgr.doc['NumHostWithMoreThanOneVote'] != 0:
            return {'pass': False, 'ids': [mmsgr.doc['_id']]}
        return {'pass': True}
