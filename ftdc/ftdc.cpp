...
void errmsg(string s, bool with_errno=true) {
    msg(0) << "error: " << s;
    if (with_errno)
...
class DataSink {
public:
    virtual void push_chunk(char* buf, char* end, Date_t id, bool interim) = 0;
};
...
private:
    ...
    void push(bool interim) {
        ...
        if (sink) {
            ...
            sink->push_chunk(out.data(), out.data() + out.size(), ref_id, interim);
...
class CompressedFileSink : public DataSink, public SampleSink {
    ...
    void push_chunk(char* buf, char* end, Date_t id, bool interim) {
        int sz = end - buf;
        if (fd >= 0) {
...
//
// write compressed bson samples as generated by built-in server code
//

class CompressedBsonSink : public DataSink, public SampleSink {
    ...
    void push_chunk(char* buf, char* end, Date_t id, bool interim) {
        ...
        BSONObjBuilder b;
        b.appendDate("_id", id);
        b.appendNumber("type", 1);
        // big hack: we don't care about size field, so let it be garbage
        int bighack = 4;
        b.appendBinData("data", sz+bighack, BinDataGeneral, buf-bighack);

        BSONObj o = b.done();

        if (interim) {
            string fn = dn + "/metrics.interim";
            int fd = open(fn.c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0666);
            if (fd < 0)
                errmsg(fn.c_str());
            int rc = write(fd, o.objdata(), o.objsize());
            if (rc != o.objsize())
                errmsg("write");
            close(fd);
        } else {
            string fn = dn + "/metrics.0000000";
            int fd = open(fn.c_str(), O_CREAT | O_WRONLY | O_APPEND, 0666);
            if (fd < 0)
                errmsg(fn.c_str());
            int rc = write(fd, o.objdata(), o.objsize());
            if (rc != o.objsize())
                errmsg("write");
            close(fd);
            rc = unlink((dn + "/metrics.interim").c_str());
            if (rc < 0)
                errmsg("unlink");
        }
...
public:
    ...
    CompressedBsonSink(string dn, int chunk_size, int chunk_update_size) :
        space("compressed bson sink " + dn)
    {
        this->dn = dn;
    ...
    void put_sample(BSONObj& sample) {
        compress->put_sample(sample, now_ms());
        space.record_sample(1, 0);
    }
};
...
int main(int argc, char* argv[]) {
    ...
    for (int i=1; i<argc; i++) {
        if (argv[i]==string("-n"))
           ...
           delay = atof(argv[++i]);
            ...
            // defaults to true now, so not useful any more
            do_repl = true;
    ...
    if (ends_with(source_spec, ".bson")) {
        ...
        if (source_spec.find("?ns") != string::npos) {
            ...
            source.reset(new LiveSource(source_spec, do_repl));
            if (!delay)
                delay = 1.0;
        }
    ...
    if (ends_with(sink_spec, ".bson")) {
        ...
        // assume compressed bson sink directory (like mongod $dbpath/diagnostic.data)
        sink.reset(new CompressedBsonSink(sink_spec, chunk_size, chunk_update_size));
    }
